...
class PythonAPIAnalyzer:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Call):
                    ...
                    func_info = {
                        ...
                        'full_signature': self.full_signature(node),
    ...
    def save_results_as_csv(self, results: List[Dict[str, Any]], output_dir: str, filename: str = None):
        ...
        # CSV列顺序
        fieldnames = [
            'callstr', 'package', 'clazz', 'full_signature', 'internal_signature',
            'func', 'is_static', 'file', 'location', 'parameter_types', 'return_type', 'doc'
        ]
        
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            # 使用quoting=csv.QUOTE_ALL确保所有字段都用双引号包裹
        ...
        # 显示一些统计信息
        calls = [r for r in results if not r['full_signature']]  # 调用没有完整签名
        functions = [r for r in results if r['full_signature']]  # 函数定义有完整签名
        
        print(f"Function calls found: {len(calls)}")
        print(f"Function definitions found: {len(functions)}")
